...
class BenPieChart extends Chart {
    ...
    getSubset() {
        ...
        for (var x in this.fullSet) {
            if(this.fullSet[x].beneficiaryType==this.benType) {
                if(this.fullSet[x].sector == 'Total') {
    ...
    */
    ...
    /* Draws the doughnut of the sector slices
    */
    drawDoughnut() {
        ...
        var arc = this.chart.benDoughnut
            ...
            .on('click', function(d) {
                updateBenBarChart(benType, d.data.sector)
            });
        ...
        arc.append('path')
            ...
            .attr('class', function(d) { return d.data.sector})
...
class BenBarChart extends Chart {
    ...
    */
    getSubset() {
        ...
        for (var x in this.fullSet) {
            if(this.fullSet[x].beneficiaryType==this.benType) {
                if(this.fullSet[x].sector == this.sector) {
                    this.subSet.push(this.fullSet[x]);
                }
            }
        }
    ...
    /* Updates the chart using the new beneficiary type
       and sector
    */
    updateChart(newBenType, newSector) {
        // Set the new benType
        this.benType = newBenType;
        // Set the new sector
    ...
    */
    drawTitle() {
        ...
        this.title
            ...
            .append('text')
    		.classed(`title ${this.sector}`, true)
            // Set the text to the title
    		.text(`${this.sector} ${this.benType}`)
...
d3.select('#benSelect')
    .on('change', function() {
        ...
        // Update the charts
        benBar.updateChart(newBenType, benBar.sector);
        benPie.updateChart(newBenType);
    });

// Update the beneficiary bar chart when the pie chart
// sector is clicked
function updateBenBarChart(newBenType, sector) {
    benBar.updateChart(newBenType, sector);
}
