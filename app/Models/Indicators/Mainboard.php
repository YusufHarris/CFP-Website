...
class Mainboard extends Model
{
    ...
    {
        $sqlQuery = 'SELECT "Stayed the Same" AS category, SUM(IF( (  ( (LENGTH(`id_LK_KeyActivitiesAwareness`) - LENGTH(REPLACE(`id_LK_KeyActivitiesAwareness`, "\n", ""))) - (LENGTH(`id_LK_KeyActivitiesAwarenessBefore`) - LENGTH(REPLACE(`id_LK_KeyActivitiesAwarenessBefore`, "\n", ""))) = 0))
                                                          ,1,0)) AS amount
                     FROM `SV_Continuous`
                    ';
                    $sqlQuery2 =  'SELECT "Increased" AS category, SUM(IF( (
